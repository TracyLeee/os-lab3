...
/* Purpose: Writes values back to the underlying disk.
 * Arguments:
 *  off - the offset to write the buffer at
 *  usr_buf - pointer to a buffer whose data will be
 *  written to the underlying disk
 *
 * Returns:
 *  -2 if the offset is invalid
 *  -1 if the underlying disk system failed with an error
 *  0 otherwise on success
 */
int put_block(disk_off_t off, void *usr_buf);
